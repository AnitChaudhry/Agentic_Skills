 ...
 * Dynamically uses prompts and skills for enhanced responses.
 */
 ...
 * Uses matched prompts when available
 */
function generateResponse(profileId, userMessage) {
  const lowerMessage = userMessage.toLowerCase();

  // Try to match a prompt first
  ...
  // Fall back to intent-based detection
...
function generateGeneralResponse(profileId, userMessage) {
  ...
  if (!profile.found) {
    return `Hey! I'm your OpenAnalyst accountability coach. I don't see a profile set up for you yet. Let's get started!`;
  }
  ...
  if (lowerMessage.match(/^(hi|hey|hello|yo|sup|what's up|howdy)/)) {
    if (summary.totalTodos === 0 && summary.totalChallenges === 0) {
      return `Hey ${name}! Good to see you. You don't have any active tasks or challenges yet. Want to set something up?`;
    }
  ...
  return `Hey ${name}! I'm here to help you stay accountable. Ask me about your tasks, challenges, or progress - or just tell me what's on your mind.`;
}
...
function generateUnifiedResponse(userMessage) {
  ...
  if (matchedPrompt) {
    ...
    // Use first profile for unified or generate without profile-specific data
