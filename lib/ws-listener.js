...
async function handleIncomingChat(message) {
  ...
  // Don't auto-respond with templates - Claude Code will respond directly
  // The message is saved to pending folder and logged above
  // Claude Code sees "[CHAT]" log and responds via send-response-fast.js
  console.log(`[WAITING] Request ${requestId} ready for Claude Code response`);
  console.log(`[WAITING] Run: node send-response-fast.js ${requestId}`);
}
